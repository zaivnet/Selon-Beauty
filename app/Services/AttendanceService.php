...
class AttendanceService
{
    ...
    {
        return DB::transaction(function () use ($actor, $evidence) {
            ...
            // Validate & Process Selfie Evidence
            ...
            $selfiePath = $this->selfieService->processAndStore($selfieInput, $employee->id, 'check_in');
            ...
            // Authoritative server timestamp (Asia/Jakarta)
            $serverNow = Carbon::now('Asia/Jakarta');

            // Shift start datetime
            $shiftStart = Carbon::parse($workDateStr.' '.$schedule->shift->start_time, 'Asia/Jakarta');
            ...
            if ($serverNow->greaterThan($graceLimit)) {
                $lateMins = (int) floor($shiftStart->diffInMinutes($serverNow, false));
    ...
    public function checkOut(User $actor, array $evidence = []): AttendanceRecord
    {
        return DB::transaction(function () use ($actor, $evidence) {
            ...
            $selfiePath = $this->selfieService->processAndStore($selfieInput, $employee->id, 'check_out');
            ...
            // Calculate shift start and shift end datetimes in Asia/Jakarta
            $shiftStart = Carbon::parse($workDateStr.' '.$schedule->shift->start_time, 'Asia/Jakarta');
            $shiftEnd = Carbon::parse($workDateStr.' '.$schedule->shift->end_time, 'Asia/Jakarta');
        ...
        return DB::transaction(function () use ($record, $checkInStr, $checkOutStr, $status, $reason, $actor) {
            ...
            if ($shift && $checkInAt) {
                ...
                if ($checkInAt->greaterThan($graceTime)) {
                    $lateMinutes = (int) floor($shiftStart->diffInMinutes($checkInAt, false));

    ...
    <div class="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-4 transition-colors">
        ...
        <form method="GET" action="{{ route('admin.leave-requests.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
            ...
            <div class="w-full min-w-0 max-w-full">
                <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Tanggal Mulai</label>
                <x-date-input name="start_date" value="{{ $filters['start_date'] }}" wrapperClass="bg-white dark:bg-slate-950" />
            </div>

            <div class="flex items-end gap-2">
                <button type="submit" class="flex-1 py-2.5 px-4 bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white font-extrabold rounded-xl transition-colors shadow-xs flex items-center justify-center gap-1 cursor-pointer min-h-[44px]">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
                    <span>Filter</span>
                </button>
                <a href="{{ route('admin.leave-requests.index') }}" class="py-2.5 px-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-extrabold rounded-xl transition-colors cursor-pointer text-center min-h-[44px] flex items-center justify-center">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 md:p-6 space-y-4 transition-colors">
        @if(count($requests) === 0)
            <div class="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-800/40">
                ...
                <h4 class="text-sm font-bold text-slate-800 dark:text-slate-200">Tidak Ada Pengajuan Ditemukan</h4>
                <p class="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto mt-1">Tidak ada data pengajuan yang sesuai dengan kriteria filter saat ini.</p>
            </div>
        @else
            <!-- Desktop Table View -->
                            ...
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                                <td class="py-3.5 px-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-extrabold text-slate-900 dark:text-slate-100">{{ $req->employee->full_name }}</p>
                                            <p class="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{{ $req->employee->employee_code }} • {{ $req->employee->jobTitle?->name ?? 'Staf' }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-4 max-w-xs truncate text-slate-700 dark:text-slate-300" title="{{ $req->reason }}">
                                    "{{ $req->reason }}"
                                </td>
                                <td class="py-3.5 px-4">
                                    @if($req->attachment_path)
                                        <a href="{{ route('leave-requests.attachment', $req->id) }}" target="_blank" class="inline-flex items-center gap-1 font-bold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 text-[11px]">
                                ...
                                <td class="py-3.5 px-4 text-right space-x-1">
                                    ...
                                    @else
                                        <span class="text-[11px] text-slate-400 dark:text-slate-500 font-medium italic">Diproses oleh {{ $req->reviewer?->name ?? 'System' }}</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                ...
                @foreach($requests as $req)
                    <div class="p-4 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 rounded-xl space-y-3">
                        <div class="flex items-center justify-between">
                            <div>
                                <h4 class="text-xs font-extrabold text-slate-900 dark:text-slate-100">{{ $req->employee->full_name }}</h4>
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 font-mono">{{ $req->employee->employee_code }} • {{ $req->type_label }}</p>
                            </div>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border {{ $req->status_badge_class }}">
                                {{ $req->status_label }}
                            </span>
                        ...
                        <div class="text-[11px] space-y-1 border-t border-b border-slate-200/60 dark:border-slate-700/60 py-2 font-mono">
                            ...
                            <p class="text-slate-600 dark:text-slate-400 font-sans italic">"{{ $req->reason }}"</p>
                        </div>

                        <div class="flex items-center justify-between pt-1">
                            <div>
                                @if($req->attachment_path)
                                    <a href="{{ route('leave-requests.attachment', $req->id) }}" target="_blank" class="font-bold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 text-[11px]">
                                        📎 Lampiran
                                    </a>
                                @endif
                            </div>
...
<div id="modal-approve" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs hidden flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 dark:border-slate-800">
        <h3 class="text-base font-extrabold text-slate-900 dark:text-slate-100">Setujui Pengajuan</h3>
        <p id="approve-modal-target" class="text-xs text-slate-600 dark:text-slate-400"></p>

        <form id="approve-form" method="POST" action="" class="space-y-4 text-xs">
            @csrf
            <div>
                <label class="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Catatan Persetujuan (Opsional)</label>
                <textarea name="reviewer_note" rows="2" placeholder="Catatan untuk karyawan..." class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"></textarea>
            </div>

            <div class="flex items-center justify-end gap-2">
                <button type="button" onclick="closeApproveModal()" class="px-3.5 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700">
                    Batal
                </button>
...
<div id="modal-reject" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs hidden flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 dark:border-slate-800">
        <h3 class="text-base font-extrabold text-slate-900 dark:text-slate-100">Tolak Pengajuan</h3>
        <p id="reject-modal-target" class="text-xs text-slate-600 dark:text-slate-400"></p>
        ...
        <form id="reject-form" method="POST" action="" class="space-y-4 text-xs">
            ...
            <div>
                ...
                <textarea name="reviewer_note" rows="3" required placeholder="Tuliskan alasan penolakan agar karyawan dapat memahami..." class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"></textarea>
            </div>

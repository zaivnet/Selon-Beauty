...
<div class="space-y-5 md:space-y-6">
    ...
    <section aria-labelledby="filter-title" class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xs md:p-5 transition-colors">
        ...
        <form method="GET" action="{{ route('admin.operational-exceptions.index') }}" class="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-5" aria-label="Filter operational exceptions">
            ...
            <div class="min-w-0"><label for="severity" class="mb-1.5 block text-[10px] font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-300">Severity</label><select id="severity" name="severity" class="min-h-11 w-full min-w-0 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"><option value="">Semua severity</option><option value="critical" @selected($filters['severity'] === 'critical')>Critical</option><option value="warning" @selected($filters['severity'] === 'warning')>Warning</option><option value="info" @selected($filters['severity'] === 'info')>Info</option></select></div>
            <div class="min-w-0"><label for="category" class="mb-1.5 block text-[10px] font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-300">Kategori</label><select id="category" name="category" class="min-h-11 w-full min-w-0 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"><option value="">Semua kategori</option>@foreach($categories as $key => $category)<option value="{{ $key }}" @selected($filters['category'] === $key)>{{ $category['label'] }}</option>@endforeach</select></div>
            <div class="min-w-0"><label for="employee_id" class="mb-1.5 block text-[10px] font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-300">Karyawan</label><select id="employee_id" name="employee_id" class="min-h-11 w-full min-w-0 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"><option value="">Semua karyawan</option>@foreach($employees as $employee)<option value="{{ $employee->id }}" @selected((string) $filters['employee_id'] === (string) $employee->id)>{{ $employee->full_name }}</option>@endforeach</select></div>
            <div class="min-w-0"><label for="job_title_id" class="mb-1.5 block text-[10px] font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-300">Jabatan</label><select id="job_title_id" name="job_title_id" class="min-h-11 w-full min-w-0 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 text-xs font-bold text-slate-800 dark:text-slate-100 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/20"><option value="">Semua jabatan</option>@foreach($jobTitles as $jobTitle)<option value="{{ $jobTitle->id }}" @selected((string) $filters['job_title_id'] === (string) $jobTitle->id)>{{ $jobTitle->name }}</option>@endforeach</select></div>
            <div class="grid grid-cols-2 gap-2 sm:col-span-2 lg:col-span-5 lg:ml-auto lg:w-72"><a href="{{ route('admin.operational-exceptions.index') }}" class="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 text-xs font-extrabold text-slate-700 dark:text-slate-200 transition hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-500/20">Reset</a><button type="submit" class="min-h-11 rounded-xl bg-rose-600 px-4 text-xs font-extrabold text-white transition hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-rose-500/40 focus:ring-offset-2">Terapkan</button></div>
        </form>

...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 md:p-8 space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
            <div>
                <span class="text-xs font-mono font-bold text-rose-600 bg-rose-50 border border-rose-200 px-2.5 py-1 rounded-md">Kode: {{ $shift->code }}</span>
                <h2 class="text-xl font-extrabold text-slate-900 mt-2">{{ $shift->name }}</h2>
            </div>
            <div>
                @if($shift->is_active)
                    <span class="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-xs rounded-full">Status Aktif</span>
                @else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Jam Kerja (Start - End)</span>
                <span class="font-mono font-bold text-slate-800 text-sm mt-0.5 block">{{ $shift->formatted_work_hours }}</span>
                @if($shift->crosses_midnight)
                    <span class="text-[10px] font-bold text-indigo-700 mt-1 block">🌙 Shift Lintas Tengah Malam (Cross-midnight)</span>
                @endif
            </div>

            <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Durasi Kerja Bersih</span>
                ...
                <span class="text-[10px] text-slate-400">Total Kotor: {{ floor($shift->work_duration_minutes / 60) }}j {{ $shift->work_duration_minutes % 60 }}m | Break: {{ $shift->break_minutes }}m</span>
            </div>

            <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Grace Period (Keterlambatan)</span>
                <span class="font-bold text-emerald-700 text-sm mt-0.5 block">{{ $shift->grace_period_minutes }} Menit</span>
                <span class="text-[10px] text-slate-500">Batas toleransi masuk tepat waktu.</span>
            </div>

            <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200">
                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Durasi Istirahat</span>
                <span class="font-bold text-slate-800 text-sm mt-0.5 block">{{ $shift->break_minutes }} Menit</span>
            </div>
        </div>

        <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2 text-xs">
            <span class="font-bold text-slate-800 block uppercase tracking-wider text-[11px]">Batas Jendela Waktu Presensi:</span>

    ...
    <div class="w-full max-w-md space-y-6 relative z-10">
        ...
        <div class="text-center space-y-3">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-rose-400 uppercase tracking-widest mt-0.5">First-Run System Setup</p>
            </div>
        </div>
        ...
        <div class="bg-slate-800/90 backdrop-blur-md rounded-3xl border border-slate-700/80 shadow-2xl p-6 sm:p-8 space-y-6">
            ...
            <div class="border-b border-slate-700/80 pb-4">
                <h2 class="text-base font-extrabold text-white">Inisialisasi Superadmin Utama</h2>
            ...
            <form action="{{ route('setup.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-2 border-b border-slate-700/50">
                    <div>
                        <label for="app_name" class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Nama Aplikasi (Opsional)</label>
                        <input type="text" 
                               name="app_name" 
                               id="app_name" 
                               value="{{ old('app_name', $branding['app_name'] ?? '') }}" 
                               placeholder="e.g. Attendance Portal" 
                               class="w-full px-3.5 py-2.5 rounded-xl bg-slate-900/80 border border-slate-700 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all">
                    </div>
                    <div>
                        <label for="company_name" class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Nama Perusahaan (Opsional)</label>
                        <input type="text" 
                               name="company_name" 
                               id="company_name" 
                ...
                <!-- Nama Lengkap -->
                <div>
                    <label for="name" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">Nama Lengkap Superadmin *</label>
                    <input type="text" 
                           name="name" 
                           id="name" 
                           value="{{ old('name') }}" 
                ...
                <!-- Email -->
                <div>
                    <label for="email" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">Email Superadmin *</label>
                    <input type="email" 
                           name="email" 
                           id="email" 
                           value="{{ old('email') }}" 
                ...
                <!-- Konfirmasi Password -->
                <div>
                    <label for="password_confirmation" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">Konfirmasi Password *</label>
                    <input type="password" 
                ...
                <!-- Security Note -->
                <div class="p-3 bg-slate-900/60 rounded-xl border border-slate-700/50 text-[11px] text-slate-400 leading-normal">
                    🔒 Role <strong class="text-rose-400">Superadmin</strong> dan status <strong class="text-emerald-400">Aktif</strong> ditentukan secara otomatis oleh server. Akun ini tidak terikat dengan data karyawan operasional toko.
                </div>
